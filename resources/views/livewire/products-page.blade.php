                                ...
                                @foreach ($categories as $category)
                                    <li wire:key="{{ $category->id }}" class="mb-4">
                                        <label for="{{ $category->slug }}"
                                            ...
                                            <input id="{{ $category->slug }}" value="{{ $category->id }}"
                                                wire:model.live="selected_categories" type="checkbox"
                                                class="w-4 h-4 mr-2">
                        ...
                        <div class="p-4 mb-5 bg-white border border-gray-200 dark:bg-gray-900 dark:border-gray-900">
                            ...
                            <ul>
                                @foreach ($brands as $item)
                                    <li wire:key="{{ $item->id }}" class="mb-4">
                                        <label for="{{ $item->slug }}" class="flex items-center dark:text-gray-400">
                                            <input wire:model.live="selected_brands" id="{{ $item->slug }}"
                                                value="{{ $item->id }}" type="checkbox" class="w-4 h-4 mr-2">
                            ...
                            <ul>
                                <li class="mb-4">
                                    <label for="featured" class="flex items-center dark:text-gray-300">
                                        <input wire:model.live="featured" value="1" id="featured" type="checkbox"
                                            class="w-4 h-4 mr-2">
                                ...
                                <li class="mb-4">
                                    <label for="in_stock" class="flex items-center dark:text-gray-300">
                                        <input value="1" id="in_stock" type="checkbox" class="w-4 h-4 mr-2">
                                        <span class="text-lg dark:text-gray-400">In Stock</span>
                                    </label>
                                </li>
                                <li class="mb-4">
                                    <label for="on_sale" class="flex items-center dark:text-gray-300">
                                        <input value="1" id="on_sale" type="checkbox" class="w-4 h-4 mr-2">
                        ...
                        <div class="p-4 mb-5 bg-white border border-gray-200 dark:bg-gray-900 dark:border-gray-900">
                            ...
                            <div>
                                ...
                                <input wire:model.live="price_range" type="range"
                                    ...
                                    max="1000" value="10000" step="100">
                                <div class="flex justify-between ">
                                    <span
                                        class="inline-block text-lg font-bold text-blue-400 ">{{ Number::currency(100, 'USD') }}</span>
                                    <span
                                        class="inline-block text-lg font-bold text-blue-400 ">{{ Number::currency(10000, 'USD') }}</span>
                                </div>
                            </div>
                        </div>
                    </div>
                                    ...
                                    <div class="border border-gray-300 dark:border-gray-700">
                                        <div class="relative bg-gray-200">
                                            <a href="/products/{{ $product->slug }}" class="">
                                                <img src="{{ isset($product->image) && is_string($product->image) ? asset('storage/' . $product->image) : (is_array($product->image) && isset($product->image[0]) ? asset('storage/' . $product->image[0]) : 'https://via.placeholder.com/80') }}"
                                                    alt="{{ $product->name }}"
